 ...
 * defined in its internal bean factory: Therefore, this class automatically registers
 ...
 *
 * <p>
 * A {@link org.springframework.context.MessageSource} may also be supplied as a bean in the
 ...
 * "applicationEventMulticaster" bean of type
 * {@link org.springframework.context.event.ApplicationEventMulticaster} in the context; otherwise,
 * a default multicaster of type
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
     ...
     * <p>
     * Default is the object id of the context instance, or the name of the context bean if the
     ...
     * <p>
     * Default value is determined by {@link #createEnvironment()}. Replacing the default with this
     * method is one option but configuration through {@link #getEnvironment()} should also be
     * considered. In either case, such modifications should be performed <em>before</em>
     * {@link #refresh()}.
     * 
     ...
     * <p>
     * Note: Listeners get initialized after the MessageSource, to be able to access it within
     ...
     * instances. Default is a
     * {@link org.springframework.core.io.support.PathMatchingResourcePatternResolver}, supporting
     * Ant-style location patterns.
     * <p>
     * Can be overridden in subclasses, for extended resolution strategies, for example in a web
     * environment.
     * <p>
     * <b>Do not call this when needing to resolve a location pattern.</b> Call the context's
    ...
    public void refresh() throws BeansException, IllegalStateException {
        ...
        synchronized (this.startupShutdownMonitor) {
            ...
            // 初始化之前的准备工作，包括记录下容器的启动时间，标记为已启动状态
            prepareRefresh();

            // Tell the subclass to refresh the internal bean factory.
            // 将配置文件中的bean的定义解析为BeanDefinition，并以BeanName为key,beanDefinition为value，保存到注册中心中
			/**
			 * 1、创建BeanFactory对象
			 * 2、xml解析
			 * 	传统标签解析：bean、import等
			 * 	自定义标签解析，有前缀的就是自定义的标签：如	<context:component-scan base-package="com.harvey.spring"/>
			 * 	自定义标签解析流程：
			 * 		a、根据当前解析标签的头信息找到对应的namespaceUri
			 * 		b、加载Spring中所有jar包下的spring.handers文件，建立映射关系
			 * 		c、根据namespaceUri从映射关系中找到对应的实现了NamespaceHnadler接口的类
			 * 		d、调用类的init方法，注册各种自定义标签的解析类
			 * 		f、根据namespaceUri找到对应的解析类,调用parser进行标签的解析
			 * 	3、把解析出来的xml标签封装成BeanDefinition对象
			 */
            ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

            // Prepare the bean factory for use in this context.
			//设置BeanFactory的类加载器，添加一些BeanProcessor,注册特殊的Bean
            prepareBeanFactory(beanFactory);

            try {
                // Allows post-processing of the bean factory in context subclasses.
				//子类可以重写该方法将自定义的BeanFactoryProcessor等加入beanFactory中，
				// 可以在初始化之前通过BeanFactory实例修改里面的内容
                postProcessBeanFactory(beanFactory);

                // Invoke factory processors registered as beans in the context.

				//将factory processors注册到上下文中，主要是执行BeanFactoryProcessor的逻辑
    ...
    protected void prepareRefresh() {
		...
		/**
		 * 初始化所有占位符相关的信息，此前Spring自己的替换已经在ClassPathXmlApplicationContext中进行，
		 * 这个地方主要是为其他项目进行扩展使用，比如web项目加载自己的文件
		 */
        initPropertySources();
    ...
    /**
     *
     * 将配置文件中的bean的定义解析为BeanDefinition，并以BeanName为key,beanDefinition为value，保存到注册中心中 <br/>
     * Tell the subclass to refresh the internal bean factory.
     * 
    ...
    protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//添加一个BeanPostProcessor,实现了Aware的接口的Bean在初始化的时候，这个Processor负责进行回调
        beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));

        //如果bean依赖下面的接口，在自动装配的时候会忽略，spring通过其他的方式处理依赖
		...
		//在Bean实例化后，如果是ApplicationListener的子类，会加入listener列表
    ...
    protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
    	...
    	//将BeanFactoryPostProcessor的处理交给PostProcessorRegistrationDelegate
    ...
    protected void initLifecycleProcessor() {
        ...
        //如果beanFactory中存在lifecycleProcessor，则通过容器创建lifecycleProcessor对象
        if (beanFactory.containsLocalBean(LIFECYCLE_PROCESSOR_BEAN_NAME)) {
        	...
        	//否则创建一个DefaultLifecycleProcessor对象，并赋值给beanFactory
    ...
    protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		//如果存在conversionService，则初始化conversionService
     ...
     * <p>
     * The {@link #close()} method is the native way to shut down an ApplicationContext, which this
     ...
     * <p>
     * Delegates to {@code doClose()} for the actual closing procedure. Also removes a JVM shutdown
     ...
     * <p>
     * Can be overridden to add context-specific bean destruction steps right before or right after
     ...
     * <p>
     * Called at the end of {@link #doClose}'s shutdown procedure, after this context's BeanFactory
     ...
     * <p>
     * Invoked by all {@link BeanFactory} delegation methods that depend on an active context, i.e.
     * in particular all bean accessor methods.
     * <p>
     * The default implementation checks the {@link #isActive() 'active'} status of this context
     * overall. May be overridden for more specific checks, or for a no-op if
     ...
     * <p>
     * A subclass will either create a new bean factory and hold a reference to it, or return a
     ...
     * <p>
     * Note: Subclasses should check whether the context is still active before returning the
